# -*- coding: utf-8 -*-
"""
LTX23_Director_V2.py  ·  v15 — Ultimate Master Release
================================================================================
...
────────────────────────────────────────────────────────────────────────────
WHY EARLIER VERSIONS PRODUCED BAD VIDEO  (and what v15 fixes)
────────────────────────────────────────────────────────────────────────────
The old "Master_V2" script rebuilt the video as 5 INDEPENDENT diffusions that
were crossfaded together afterwards. That approach is NOT what the JSON does,
and it is the root cause of every symptom you reported:
    ...
    ✗ Missing Master Timeline    →  the `LTXDirector` node was absent entirely.
      Controller
    ✗ Identity / scene drift     →  5 separate samplings never share context,
                                    so the singer changes between scenes.
    ✗ Voice not synchronized     →  each fake segment got a *fresh empty* audio
                                    latent, so no real vocal track ever lined up.
    ✗ Visible seams              →  linear crossfade blending between clips.

v15 abandons the 5-segment/crossfade hack completely (as requested) and instead
reproduces the ORIGINAL graph node-for-node:
    ...
    DualCLIPLoader ─────────────────────────────────────────► (clip) ───────┤
        ...
        DECODE   : VAEDecode (video) + LTXVAudioVAEDecode (audio)
        ASSEMBLE : VHS_VideoCombine  →  single synced MP4

The ENTIRE timeline is diffused in ONE Stage-1 pass and ONE Stage-2 pass. That
is exactly why the JSON keeps a consistent character, consistent scenes and a
perfectly synchronized voice — and v15 does the same.

────────────────────────────────────────────────────────────────────────────
HOW v15 RUNS THE FULL TIMELINE WITHOUT CRASHING ON 12.2 GB FREE-TIER
────────────────────────────────────────────────────────────────────────────
Running the whole timeline single-shot is memory heavy, so v15 keeps every
"clear_memory" technique from Master_V2 and adds more:

    • 16 GB high-speed NVMe swap partition for host-RAM head-room (best effort —
      some Colab runtimes forbid swapon; the pipeline still runs without it).
    • Real model + real clip, but ComfyUI juggles them: the clip (Gemma-3-12B, ~7 GB)
      is used INSIDE LTXDirector to encode text, then freed before the 22B DiT is
      streamed to the GPU for sampling — the two are never both GPU-resident.
      (LTXDirector installs PromptRelay patches on the model and returns it; that
      SAME patched model is reused for sampling — no reload, no mock.)
    • Two-stage tiny-base diffusion: Stage 1 runs at 416x240 (very small latent)
      then a 2x latent upscaler + a light 4-step / denoise-0.42 refinement at
      832x480 — this is what makes the full timeline fit in VRAM.
    • purge_deep(): unload_all_models + cleanup + soft_empty_cache + gc +
      cuda.empty_cache + ipc_collect + OS page-cache drop + malloc_trim(0).
    • 1.2 GB dynamic VRAM shield for LoRA delta weights during sampling.
    • Out-of-core spatiotemporal TILED VAE decode (never holds the full RGB
      tensor on GPU).
    • Latents are parked on CPU between node calls; ComfyUI streams them back to
      the GPU only for the active op.
    • Resume checkpoints: Phase A+B run back-to-back (the patched model can't be
      pickled), then the final latents + decoded frames/audio are checkpointed —
      a crash during decode/assembly never loses the expensive diffusion work.

The single-pass diffusion over 756 frames is the real VRAM/RAM stress point. Use
the Cell 6 SETTINGS PANEL to fit it on a free T4: set `VRAM_MODE = "lowvram"` (streams
the 22B model so activations fit), lower `render_seconds` (e.g. 8–12 s), and turn off the
stylistic LoRAs (`use_lora_2/3/4 = False`). If STAGE 1 still OOMs, the definitive fix is
an L4 / A100 runtime (the workflow was authored on a desktop GPU). Re-running always
...
# expandable_segments avoids fragmentation OOM; max_split_size_mb keeps big allocs
# from wasting reserved blocks — both important for the chunked sampler on a 15 GB T4.
...
# 16 GB high-speed swap partition — critical head-room for the full timeline.
...
# ════════════════════════════════════════════════════════════════════════════
# @title {"single-column":true}
# @markdown # 💥 Cell 5: Download LTX-2.3 Models & Director 2.0 LoRAs
import torch
...
# ════════════════════════════════════════════════════════════════════════════
# CELL 6: COMPLETE TIMELINE "NOTES" — GLOBAL PROMPT, TRACKS & LTXDIRECTOR WIDGETS
# ════════════════════════════════════════════════════════════════════════════
# @title {"single-column":true}
# @markdown # 💥 Cell 6: Master Timeline Configuration (all notes / tracks / widgets)
#
# Every value below is transcribed 1:1 from the LTXDirector node (id 131) inside
# LTX-2.3_Director_2.0-MV-Workflow-30s.json — nothing is invented. This is the
# full set of "notes" that was missing in the old Master_V2 script.
...
# ════════════════════════════════════════════════════════════════════════════
#  🎛️  USER SETTINGS PANEL  (Colab @param form — edit here, then run Cell 17)
# ════════════════════════════════════════════════════════════════════════════
...
# @markdown `render_seconds` — lower this (e.g. 8–12 s) to shrink VRAM/RAM use on a free T4.
...
# @markdown ## 🎛️ Director 2.0 LoRA Stack  (turn OFF LoRAs to avoid the GGUF dequant RAM spike)
...
use_lora_4 = False     # @param {type:"boolean"}
...
# @markdown ## 🎵 Audio (synchronized voice track)
use_song_audio = True                       # @param {type:"boolean"}
audio_trim_start_frames = 446.9222739141953  # @param {type:"raw"}

# @markdown ## 🧠 Memory / Performance  (free-tier T4 — 15 GB VRAM / 12 GB RAM)
# @markdown `VRAM_MODE` is the key knob for the T4:
# @markdown • **auto** — DO NOT force any mode; let ComfyUI do its smart *partial* offload (most of
# @markdown   the model on GPU, overflow on CPU). This is EXACTLY how V2 runs at ~40 s/it. USE THIS.
# @markdown • **normalvram** — forces whole model onto GPU → on a T4 it nearly fills VRAM and THRASHES
# @markdown   (a step can hang for many minutes). Only for GPUs with lots of VRAM.
# @markdown • **lowvram** — forces full layer-by-layer streaming (~7× slower). • **highvram** — L4/A100.
VRAM_MODE = "auto"            # @param ["auto", "normalvram", "lowvram", "novram", "highvram"]
essential_loras_only = True   # @param {type:"boolean"}   # keep only the distilled LoRA
vram_shield_mb       = 256    # @param {type:"raw"}
min_ram_guard_gb     = 1.5    # @param {type:"slider", min:1.0, max:6.0, step:0.5}
# @markdown ### 🎬 Batch-by-batch scene generation (fits the T4 by sampling the timeline in chunks)
# @markdown `batch_scene_mode` samples the ONE shared LTXDirector timeline in overlapping temporal
# @markdown chunks (deep memory purge between each) instead of one giant pass — the only way a 22B
# @markdown model fits a T4. Overlap frames are cross-faded so scenes stay smooth. Turn OFF only on
# @markdown an L4/A100 to do the true single pass.
batch_scene_mode          = True   # @param {type:"boolean"}
# scene_mode: "keyframe" = one scene per keyframe (5 scenes, exactly like the proven V2 sizes);
#             "fixed"    = uniform chunks of scene_chunk_seconds.
...
# reload_dit_per_scene: fully purge + reload the DiT before EACH scene (V2's winning
# pattern — resets VRAM/RAM every scene so nothing accumulates). Recommended on a T4.
...
# ────────────────────────────────────────────────────────────────────────────
#  Derived configuration (built from the settings above — do not edit below).
# ────────────────────────────────────────────────────────────────────────────
...
TIMELINE_METADATA = {
    ...
    "generation_width": int(generation_width),      # Stage-2 refined render res
    "generation_height": int(generation_height),
    "base_stage1_width": int(generation_width) // 2,  # Stage-1 base (= gen / 2)
    ...
    "local_prompts": "",   # empty → LTXDirector single-prompt bypass path
}

# Director 2.0 LoRA stack — built from the panel toggles/strengths.
_ALL_LORAS = [
...
if not LORA_STACK:
    print("  ⚙️ All LoRAs disabled — base GGUF sampling (no dequant RAM spike; consider more steps).")
...
# Runtime globals consumed by the memory engine (Cell 8) & loaders (Cell 11) & run (Cell 17).
...
# chunk size in LATENT frames (LTX temporal compression = 8): pixel≈(lat-1)*8+1
...
print(f"✅ Cell 6: Settings applied → {generation_width}x{generation_height} @ {fps}fps · "
      f"{_duration_seconds:.1f}s ({_total_frames} frames) · "
      f"{len(LORA_STACK)} LoRA(s) · VRAM_MODE={VRAM_MODE} · "
      f"essential_loras_only={ESSENTIAL_LORAS_ONLY}")
...
# ════════════════════════════════════════════════════════════════════════════
# @title {"single-column":true}
# @markdown # 💥 Cell 7: Register ComfyUI nodes & verify every original node exists
import asyncio
import nest_asyncio
nest_asyncio.apply()
...
# ════════════════════════════════════════════════════════════════════════════
# CELL 8: PRODUCTION MEMORY ENGINE (clear_memory / purge_deep / VRAM shield)
# ════════════════════════════════════════════════════════════════════════════
# @title {"single-column":true}
# @markdown # 💥 Cell 8: Zero-Crash Memory Engine
def malloc_trim_os():
    try:
...
def patch_comfy_memory_manager():
    """Fix free_memory return type + reserve a 1.2 GB VRAM shield for LoRA deltas."""
    try:
        ...
        if not getattr(mm, "_ltx_patched", False):
            ...
            def _buffered_getfree(dev=None, torch_free_too=False):
                try:
                    ...
                    # Small 256 MB buffer only. A large shield made ComfyUI UNDER-
                    # estimate free VRAM and offload the GGUF weights to HOST RAM,
                    # where dequantizing 22B → bf16 blows the 12 GB RAM. We want the
                    # opposite: keep weights on the GPU.
        ...
        # CRITICAL for free-tier: force the text encoder onto the GPU so the
        # 12B Gemma model dequantizes into VRAM (15 GB) instead of the 12 GB host
        # RAM. Without this, encoding the prompt OOMs host RAM instantly.
        if torch.cuda.is_available():
...
def light_clear():
    """Fast, cheap cache flush safe to call VERY frequently (e.g. every diffusion
    step) — no model unloading, just free fragmented CUDA blocks + a gc pass."""
    gc.collect()
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
...
def medium_clear(tag: str = ""):
    """Between-sub-step clear: soft cache flush + IPC collect + malloc_trim, but
    WITHOUT unloading the model (so mid-scene it doesn't force a costly reload).
    Called after every heavy sub-step (guide / sample / upscale) for tight control."""
    try:
        import comfy.model_management as mm
        mm.soft_empty_cache()
    except Exception:
...
def install_sampling_memory_hook(clear_every: int = 1, ram_guard_gb: float = 0.0):
    """
    Register a ComfyUI progress-bar hook that runs a LIGHT memory clear on every
    (or every Nth) diffusion step. This is the 'automatic memory clearing during
    generation' — it keeps CUDA fragmentation from accumulating across the 8-step
    full-timeline sample, which is a common cause of mid-sample OOM on a T4.
    """
    try:
        ...
        def _hook(value, total, preview_bytes=None, *args, **kwargs):
            # ComfyUI may call this with extra kwargs (e.g. node_id=...) — accept all.
            state["n"] += 1
...
def remove_sampling_memory_hook():
    try:
        import comfy.utils as cu
        if hasattr(cu, "set_progress_bar_global_hook"):
...
def configure_vram_state(mode: str = "lowvram"):
    """
    Pick the ComfyUI VRAM strategy. The T4 (14.56 GB) cannot hold the whole 22B
    GGUF model AND the 756-frame activations, so the right choice is LOW_VRAM:
    ComfyUI streams the model to the GPU layer-by-layer, keeping only the active
    layer resident and freeing the rest of VRAM for activations (slower, but fits).

      lowvram    → stream layer-by-layer (USE THIS on a free T4)
      highvram   → keep whole model on GPU (needs L4/A100; OOMs a T4)
      normalvram → ComfyUI's default heuristic
      novram     → most aggressive streaming (slowest, smallest footprint)
      auto       → leave ComfyUI's auto-detected state untouched
    """
    try:
        import comfy.model_management as mm
        if not torch.cuda.is_available():
            return
        mode = (mode or "lowvram").lower()
        dev = mm.get_torch_device()

        # Remember the original offload getters so non-highvram modes can restore them.
        if not hasattr(mm, "_ltx_orig_unet_offload"):
            mm._ltx_orig_unet_offload = mm.unet_offload_device
        mm.unet_offload_device = mm._ltx_orig_unet_offload  # default: offload to CPU

        vs = getattr(mm, "VRAMState", None)
        if vs is None:
            print("  [vram-state notice] VRAMState enum not found; leaving default.")
            return

        if mode == "highvram":
            mm.vram_state = vs.HIGH_VRAM
            mm.unet_offload_device = lambda: dev   # keep weights on GPU
            print("  ⚙️ VRAM → HIGH_VRAM (whole model on GPU — needs L4/A100).")
        elif mode == "lowvram":
            mm.vram_state = vs.LOW_VRAM
            print("  ⚙️ VRAM → LOW_VRAM (model streams layer-by-layer → room for activations on the T4).")
        elif mode == "novram":
            mm.vram_state = vs.NO_VRAM
            print("  ⚙️ VRAM → NO_VRAM (maximum streaming, slowest).")
        elif mode == "normalvram":
            mm.vram_state = vs.NORMAL_VRAM
            print("  ⚙️ VRAM → NORMAL_VRAM (ComfyUI default heuristic).")
        else:
            print("  ⚙️ VRAM → auto (ComfyUI's detected state left untouched).")
    except Exception as e:
...
print("✅ Cell 8: Memory engine active (purge_deep · ram_guard · 256 MB VRAM shield · "
      "HIGH_VRAM control · page-cache drop).")
...
# ════════════════════════════════════════════════════════════════════════════
# @title {"single-column":true}
# @markdown # 💥 Cell 9: Original-node dispatcher + safe extractors
import numpy as np
...
def call_node(node_name: str, node_instance: Optional[Any] = None, **kwargs) -> Any:
    ...
    signature accepts (alias-aware, schema-default aware). This lets us drive the
    exact workflow nodes from Python without a graph executor.
    """
...
# ════════════════════════════════════════════════════════════════════════════
# @title {"single-column":true}
# @markdown # 💥 Cell 10: DirectorTimelineController — the Master Timeline note set
class DirectorTimelineController:
    ...
    def validate_reference_images(self):
        ...
        for s in self.segments:
            ...
            if not os.path.exists(full):
                # 5.3.png alias fallback to 5.png
                if "5.3.png" in s["imageFile"]:
                    ...
                    if os.path.exists(alt):
                        ...
                        continue
                # generate a labelled placeholder so the pipeline can still run
                os.makedirs(os.path.dirname(full), exist_ok=True)
    ...
    def configure_ltxdirector(self, node_instance: Any):
        ...
        # widgets_values in the exact order seen in the JSON (23 entries)
        widgets_values = [
            0,                                    # 0  seed/frame anchor
            float(m["duration_seconds"]),         # 1  start (display=seconds → total)
...
# ════════════════════════════════════════════════════════════════════════════
# CELL 11: MODEL / CLIP / LoRA LOADERS  (Power Lora Loader on model + clip)
# ════════════════════════════════════════════════════════════════════════════
# @title {"single-column":true}
# @markdown # 💥 Cell 11: DiT loader, 4-LoRA stack (model+clip) & attention hooks
def load_dit_and_loras(clip_obj: Any = None):
    """
    UnetLoaderGGUF ─► Power Lora Loader (rgthree) ─► ModelPreviewOverrideKJ hook.
    The rgthree loader applies the 4-LoRA stack to BOTH model and clip (as the
    JSON does). Falls back to LoraLoaderModelOnly if rgthree signature differs.
    Returns (model, clip).
    """
    ...
    # Memory lever: applying many LoRAs to a 22B GGUF model forces per-layer
    # dequantization (base→bf16) which is a major RAM spike during sampling.
    # ESSENTIAL_LORAS_ONLY keeps just the distilled LoRA (the one that makes
    # 8-step sampling possible) and drops the 3 stylistic ones to cut that spike.
    ...
    if not applied:
        # Deterministic per-LoRA fallback (model-only) with VRAM shield between each.
            ...
            if lc["on"] and os.path.exists(path):
                try:
                    ...
                    model = gv(call_node("LoraLoaderModelOnly", node_instance=ll,
                                         ...
                                         strength_model=lc["strength"]), 0) or model
    ...
    # ModelPreviewOverrideKJ (id 10) — pass-through in the graph; harmless if present.
...
print("✅ Cell 11: DiT + 4-LoRA (model+clip) loader with attention hooks ready.")
...
# @title {"single-column":true}
# @markdown # 💥 Cell 12: Phase A — LTXDirector Master Timeline Controller
#
# ── The free-tier RAM problem and how Phase A solves it ──────────────────────
# LTXDirector needs BOTH a real model (it installs PromptRelay patches on the
# transformer blocks) AND a clip (to encode the prompt). But co-loading the 12B
# Gemma text encoder (dequantizes to ~24 GB) with the DiT blows past 12 GB host
# RAM — that is exactly the crash you hit.
#
# Fix: fully DECOUPLE the two.
#   1) Load ONLY the clip, encode the global prompt on the GPU (text_encoder_device
#      is patched to CUDA so Gemma uses VRAM, not RAM), then PURGE Gemma completely.
#   2) Load the real GGUF DiT (mmap'd → tiny RAM) + LoRA stack (model side; the
#      Director LoRAs are diffusion LoRAs, so skipping the clip side is lossless).
#   3) Run LTXDirector with the REAL model + a PrecomputedClipProxy that just
#      returns the already-encoded conditioning — so Gemma is never reloaded and
#      the two big models are never resident at the same time.

class PrecomputedClipProxy:
    """Returns already-encoded conditioning for any text — Gemma stays purged.
    Only the clip is proxied; the MODEL passed to LTXDirector is the real one.
    The real (lightweight, weight-free) tokenizer wrapper is kept so any code path
    that inspects clip.tokenizer still works."""
    ...
    def __getattr__(self, name):
        # Any unexpected attribute access returns a callable yielding the cond.
...
def encode_prompt_on_gpu(prompt_text: str):
    ...
    while keeping the tiny (weight-free) tokenizer wrapper alive for LTXDirector.
    Returns (conditioning_on_cpu, tokenizer_wrapper)."""
    ...
    # Grab the tokenizer wrapper (vocab/merges only — no model weights) BEFORE purge.
    ...
    # Break the heavy weight references (keep the tokenizer via saved_tokenizer).
    try:
...
def execute_phase_a(timeline_ctrl: "DirectorTimelineController",
    ...
    # ── RESUME: everything Phase A produces (conditioning, latents, guide_data,
    # motion_guide_data) is picklable, so we cache it. On resume we skip the ~45 s
    # Gemma encode + timeline ingestion and return model=None (Phase B reloads the
    # DiT — in the single-prompt bypass path LTXDirector's output model is just a
    # plain clone of the DiT, so a freshly loaded DiT is equivalent).
    ...
    with torch.inference_mode():
        ...
        # A2: load the real GGUF DiT (mmap) + LoRA stack (model side only).
        ...
        # local_prompts="" → LTXDirector takes the single-prompt BYPASS path (no
        # per-segment attention masking / raw-tokenizer requirement). All 5 local
        # prompts in the source workflow are empty anyway, so the global prompt
        # drives every frame identically — keyframe guiding still happens in
        # LTXDirectorGuide. This is what lets the precomputed-clip proxy work.
        print("  🚀 Ingesting the 756-frame timeline via LTXDirector...")
        t_dir = time.time()
        ...
        # Cache Phase A so a Phase-B crash never repeats the encode/ingestion.
        try:
            tmp = state_file + ".tmp"
...
print("✅ Cell 12: Phase A (decoupled encode + real-model ingestion) ready.")



# ════════════════════════════════════════════════════════════════════════════
# CELL 13: PHASE B — SINGLE FULL-TIMELINE 2-STAGE DIFFUSION  (NO SEGMENTATION)
# ════════════════════════════════════════════════════════════════════════════
# @title {"single-column":true}
# @markdown # 💥 Cell 13: Phase B — whole-timeline Stage 1 → 2x upscale → Stage 2
#
# The exact JSON graph, run ONCE over all 756 frames (no slicing, no crossfade):
#   Stage 1 : LTXDirectorGuide(0.5) → ConcatAV → SamplerCustomAdvanced
#             (euler · linear_quadratic · 8 steps · denoise 1.0 · cfg 1)
#             → SeparateAV → LTXDirectorCropGuides → LTXVLatentUpsampler (2x)
...
#             (euler · linear_quadratic · 4 steps · denoise 0.42 · cfg 1)
#             → SeparateAV → LTXDirectorCropGuides
# One pass ⇒ consistent character, consistent scenes, and a perfectly synced voice.
...
def _save_ckpt(path: str, obj: Dict[str, Any]) -> bool:
    """Best-effort checkpoint save (tolerates non-picklable conditioning)."""
    try:
        tmp = path + ".tmp"
        torch.save(obj, tmp)
        os.replace(tmp, path)
...
def _lat_samples(latdict):
    s = unwrap_latent(latdict).get("samples", None)
    return s
...
def _keyframe_scene_ranges(Tv: int) -> List[Tuple[int, int]]:
    """One scene per keyframe (like the proven V2 sizes): split the Tv latent
    frames proportionally to the 5 keyframe segment lengths."""
...
def execute_phase_b_batched(director_state: Dict[str, Any], model: Any, seed: int,
    ...
    (same conditioning, keyframe guides and audio latent). This is the V2-proven
    pattern that fits a 22B model on a T4:

      • SCENE_MODE="keyframe" → one scene per keyframe (5 scenes, V2 sizes).
      • RELOAD_DIT_PER_SCENE  → fully purge + reload the DiT BEFORE each scene so
        VRAM/RAM reset every scene and nothing accumulates (V2's key trick).
      • medium_clear() after every heavy sub-step (guide/sample/upscale).
      • Each scene cached (scene_NN.pt) for crash-safe resume; overlaps cross-faded.
    """
    ...
    # In reload-per-scene mode we drop the model handed in by Phase A and reload
    # fresh for each scene (V2 pattern), so free it now.
    ...
    for idx, (s, e) in enumerate(scene_ranges):
        ...
        if resume and os.path.exists(ck) and os.path.getsize(ck) > 1024:
            ...
            # V2 trick: fully reset memory + reload the DiT fresh before each scene.
            ...
            with torch.inference_mode():
                # Stage 1 (base) → medium_clear → 2x upscale → medium_clear → Stage 2.
...
def _execute_phase_b_single(director_state: Dict[str, Any], model: Any, seed: int,
                            workdir: str, resume: bool = True) -> str:
    latent_file = os.path.join(workdir, "final_latents.pt")
    stage1_file = os.path.join(workdir, "stage1_latent.pt")
    upscaled_file = os.path.join(workdir, "upscaled_latent.pt")
    ...
    with torch.inference_mode():
        ...
        # ════════════ Resume tier 3: upscaled latent already exists ════════════
        if resume and os.path.exists(upscaled_file) and os.path.getsize(upscaled_file) > 1024:
            print(f"  ⏭ [RESUME] Loading cached upscaled latent: {upscaled_file}")
            ck = torch.load(upscaled_file, map_location="cpu")
            s1_pos = ck.get("s1_pos", base_pos)
            s1_neg = ck.get("s1_neg", base_neg)
            v_ups = {"samples": ck["v_ups"]}
            s1_aud = {"samples": ck["s1_aud"]} if ck.get("s1_aud") is not None else None
        else:
            # ═══════ Resume tier 2: Stage-1 output exists (skip 8-step sample) ═══
            if resume and os.path.exists(stage1_file) and os.path.getsize(stage1_file) > 1024:
                print(f"  ⏭ [RESUME] Loading cached Stage-1 latent: {stage1_file}")
                ck = torch.load(stage1_file, map_location="cpu")
                s1_pos = ck.get("s1_pos", base_pos)
                s1_neg = ck.get("s1_neg", base_neg)
                s1_vid = {"samples": ck["s1_vid"]}
                s1_aud = {"samples": ck["s1_aud"]} if ck.get("s1_aud") is not None else None
            else:
                # ── STAGE 1 (base 416x240) — the expensive 8-step sample ──
                ...
                _save_ckpt(stage1_file, {
                    "s1_vid": unwrap_tensor(s1_vid).detach().cpu().half(),
                    "s1_aud": unwrap_tensor(s1_aud).detach().cpu().half() if s1_aud is not None else None,
                    "s1_pos": sync_cond_to_cpu(s1_pos), "s1_neg": sync_cond_to_cpu(s1_neg)})
                print(f"  💾 Stage-1 latent cached: {stage1_file}")

            # ── 2x latent spatial upscale (needs the video VAE only) ──
            if video_vae is None:
                video_vae = gv(call_node("VAELoader", vae_name="LTX23_video_vae_bf16.safetensors"), 0)
            print("  ⚡ 2x latent spatial upscale...")
            up_model = gv(call_node("LatentUpscaleModelLoader",
                                    model_name="ltx-2.3-spatial-upscaler-x2-1.1.safetensors"), 0)
            v_ups = sync_latent_device(gv(call_node("LTXVLatentUpsampler",
                                                    samples=s1_vid, upscale_model=up_model, vae=video_vae), 0), "cpu")
            del up_model, s1_vid
            gc.collect()
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
            malloc_trim_os()
            _save_ckpt(upscaled_file, {
                "v_ups": unwrap_tensor(v_ups).detach().cpu().half(),
                "s1_aud": unwrap_tensor(s1_aud).detach().cpu().half() if s1_aud is not None else None,
                "s1_pos": sync_cond_to_cpu(s1_pos), "s1_neg": sync_cond_to_cpu(s1_neg)})
            print(f"  💾 Upscaled latent cached: {upscaled_file}")

        # ── STAGE 2 (refine 832x480, using the STAGE-1 sampled audio latent) ──
        model = _ensure_model(model)
        if video_vae is None:
            video_vae = gv(call_node("VAELoader", vae_name="LTX23_video_vae_bf16.safetensors"), 0)
    ...
    # Stage checkpoints are now superseded by final_latents — free the disk.
    for f in (stage1_file, upscaled_file):
        try:
            if os.path.exists(f):
                os.remove(f)
        except Exception:
...
print("✅ Cell 13: Phase B (single full-timeline 2-stage diffusion) ready.")
...
# ════════════════════════════════════════════════════════════════════════════
# @title {"single-column":true}
# @markdown # 💥 Cell 14: Phase C — tiled video decode + LTXVAudioVAEDecode
...
# ════════════════════════════════════════════════════════════════════════════
# @title {"single-column":true}
# @markdown # 💥 Cell 15: Phase D — VHS_VideoCombine master MP4
...
# ════════════════════════════════════════════════════════════════════════════
# @title {"single-column":true}
# @markdown # 💥 Cell 16: Verify final artifact
...
# ════════════════════════════════════════════════════════════════════════════
# @title {"single-column":true}
# @markdown # 💥 Cell 17: Run generation (all knobs live in the Cell 6 settings panel)
# All user settings now live in the Cell 6 @param panel:
#   BASE_SEED · OUTPUT_CRF · RESUME_CHECKPOINTS · USE_SONG_AUDIO ·
#   ESSENTIAL_LORAS_ONLY · VRAM_MODE · VRAM_SHIELD_MB · render_seconds · etc.
...
def run_ltx23_director_master(global_prompt, negative_prompt, meta, segments,
    ...
    t_start = time.time()
    print("\n" + "=" * 70 + "\n🎬 LTX-2.3 DIRECTOR 2.0 · v15 — ULTIMATE MASTER RELEASE\n" + "=" * 70)
    validate_original_nodes()
    patch_comfy_memory_manager()
    patch_safetensors_direct_to_gpu()
    configure_vram_state(mode=globals().get("VRAM_MODE", "lowvram"))
    # In LOWVRAM the model streams every step; clearing every step fights that, so
    # clear every 4 steps (still prevents fragmentation, much less overhead).
    ...
    if resume and os.path.exists(latent_file) and os.path.getsize(latent_file) > 1024:
        ...
        # Each phase self-resumes from its own cache:
        #   Phase A → director_state.pt   (skips the ~45 s Gemma encode + ingestion)
        #   Phase B → stage1_latent.pt / upscaled_latent.pt  (skips the 8-step sample)
        # so a crash mid-diffusion never repeats the work already completed.
