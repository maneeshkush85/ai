# -*- coding: utf-8 -*-
"""
LTX23_Director_Master_V5.1.py  · LTX-2.5 / LTX-5 Integration 
Target: Colab Free Tier (T4 15GB VRAM | ~12.2GB Host RAM)
Feature: Restored Colab UI Sliders (@param), 100% Memory Leak Patched
"""
...
# ════════════════════════════════════════════════════════════════════════════
# @markdown # 💥 Cell 6: LTXDirector Timeline, Resolution & Settings
# @markdown ---
...
# @markdown `render_seconds` — lower this (e.g. 8–12 s) to shrink VRAM/RAM on a free T4.
...
def patch_comfy_memory_manager():
    try:
        ...
        if not getattr(mm, "_ltx_patched", False):
            ...
            # V5 Fix: Removed the forceful text_encoder CUDA patch to allow Gemma to stream layer-by-layer
    except Exception as e:
...
def purge_deep(tag: str = ""):
    try:
        ...
        if hasattr(mm, "models_dir"): pass # Force strict GC scope
    except Exception:
        pass
    gc.collect()
    if torch.cuda.is_available():
...
def sync_cond_to_cpu(cond: Any) -> Any:
    """CRITICAL MEMORY FIX: Ensures no PyTorch CUDA tensors are trapped in dicts/lists 
    before saving to .pt state files. Stops the 14GB VRAM lock issue."""
    if cond is None:
        return None
...
def encode_prompt_on_gpu(prompt_text: str):
    ...
    # Aggressive Deep Clears to recover Text Encode memory
    try:
        mm.unload_all_models()
        mm.cleanup_models()
        mm.soft_empty_cache()
...
def execute_phase_a(timeline_ctrl: "DirectorTimelineController",
    ...
    with torch.inference_mode():
        ...
        # 🛡️ THE FIX: Ensure NO PyTorch variables leak into the cached dictionary.
        # sync_cond_to_cpu forces the detachement and deep-copy to CPU ram.
